...
Past context adds compute but no algorithmic latency. On six Torch CPU threads, including log-mel extraction and the deliberately uncached overlap, measured median replay RTF is **0.0094** (about 106× real time). Routing policy smoothing/dwell is separate from model latency.
...
| Six-thread CPU replay RTF | 0.0094 |
