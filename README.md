...
Equal clip counts did not produce balanced supervision. Audit schema 2 applies the actual `D=21`, `L=4`, and 100-frame ramp to all 71 training targets. The per-language probabilities below are one-pass full-corpus frame/ramp-weighted means over the ten monolingual source clips, while target mass is the corresponding share of all T=2 target probability mass, including the mixed clip. This is a corpus surrogate, not the realized exposure from shuffled, independently normalized, drop-last minibatches.

| Manifest language | Clips | Seven-way teacher top-1 correct | Full-corpus T=1 P(label) | Full-corpus T=2 P(label) | Full-corpus T=2 target mass |
|---|---:|---:|---:|---:|---:|
...
| Training-target audit / diagnostic floors | 71/71 clips recomputed; valid schema-2 ledger; English selected 6/10 and native 1/10 (<8/10) |
...
| Six-thread offline-replay compute RTF | 0.0056 median (process-local) |
...
- `tests/test_target_cache.py` rejects reordered class columns, changed waveform/manifest/config content, modified target files, forged availability ledgers, dense tensors that disagree with their declared hold expansion, soft anchors that disagree with `T=2`, impossible native indices/labels/probabilities, and a self-rehashed forged training-target audit; it also proves exact-byte versus semantic manifest identity, defensive copies, cache/corpus/dataset construction without reopening the manifest, and equal-count/unequal-supervision plus negligible-retained-mass cases.
